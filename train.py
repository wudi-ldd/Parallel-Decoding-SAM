...
def set_seed(seed):
    """Seed Python, NumPy, and PyTorch RNGs for deterministic training."""
    random.seed(seed)
    np.random.seed(seed)
...
trainable_params = lora_trainable_params + model_trainable_params
...
def lr_lambda(epoch):
    """Cosine schedule with warmup for the shared optimizer."""
...
for epoch in range(num_epochs):
    ...
    for images, masks in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs} [Train]"):
        ...
        # Forward pass - only get final image embedding
        image_embedding = lora_sam_model.sam.image_encoder(images)  # grad flows into LoRA
        seg_head_logits = model(image_embedding)

        prompts = process_class_logits(seg_head_logits)

        # NOTE: Allow gradients from the SAM mask decoder to reach LoRA parameters (no `no_grad`).
        final_predictions = predict_masks_batch(
            lora_sam_model.sam, image_embedding, prompts, CONFIG['device']
        )

        # Compute loss - simplified without auxiliary loss
        loss, seg_loss_val, sam_loss_val = compute_loss(
    ...
    with torch.no_grad():
        for images, masks in tqdm(val_loader, desc=f"Epoch {epoch + 1}/{num_epochs} [Validation]"):
            ...
            # Forward pass - only get final image embedding
            image_embedding = lora_sam_model.sam.image_encoder(images)
            seg_head_logits = model(image_embedding)
    ...
    # Compute composite score (for logging purposes)
    composite_score = (
        avg_iou * weights['iou'] +
        avg_f1 * weights['f1'] +
    ...
    # Save best model based on final IoU (changed from composite score)
    if avg_iou > best_final_iou:
        best_final_iou = avg_iou
        best_epoch = epoch + 1

        lora_path = os.path.join(save_dir, f"{CONFIG['save_prefix']}_lora.safetensors")
        checkpoint_path = os.path.join(save_dir, f"{CONFIG['save_prefix']}.pth")
        ...
        save_message = (
            f"Best model saved at epoch {best_epoch} with Final IoU {best_final_iou:.4f}"
        )
